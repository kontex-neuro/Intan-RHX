    ...
    maxReadLength(maxReadLength_)
{
    ...
    cout << "DataStreamFifo: Allocating " << 2 * bufferSizeWithExtra / 1.0e6 << " MBytes for FIFO buffer." << endl;
    buffer = nullptr;
    ...
    try {
        ...
        cerr << "Error: DataStreamFifo constructor could not allocate " << memoryNeededGB << " GB of memory." << endl;
    }

    if (!buffer) {
        cerr << "Error: DataStreamFifo constructor could not allocate " << 2 * bufferSizeWithExtra << " bytes of memory." << endl;
    }
    resetBuffer();
}
