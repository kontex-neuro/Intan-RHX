...
unsigned int RHXDataBlock::dataBlockSizeInWords(ControllerType type_, int numDataStreams_)
{
    ...
    case ControllerRecordUSB3:
        return samplesPerDataBlock(type_) * (4 + 2 + (numDataStreams_ * (channelsPerStream(type_) + numAuxChannels(type_))) + ((numDataStreams_ + false * 2) % 4) + 8 + 2 + false * 2);
        // 4 = magic number; 2 = time stamp; 35 = (32 amp channels + 3 aux commands); 0-3 filler words; 8 = ADCs; 4 = TTL in/out
    case ControllerStimRecord:
        return samplesPerDataBlock(type_) * (4 + 2 + numDataStreams_ * (2 * (channelsPerStream(type_) + numAuxChannels(type_)) + 4) + false * 2 + 8 + 8 + 2 + false * 2);
